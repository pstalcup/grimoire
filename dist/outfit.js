import { canEquip, equip, equippedAmount, equippedItem, Item, weaponHands as mafiaWeaponHands, toSlot, useFamiliar, } from "kolmafia";
...
export class Outfit {
    ...
    equipItem(item) {
        var _a;
        if (!have(item) || !canEquip(item))
            return false;
        if ((_a = this.avoid) === null || _a === void 0 ? void 0 : _a.find((i) => i === item))
            return false;
        ...
        switch (slot) {
            ...
            case $slot `off-hand`:
                ...
                break;
            case $slot `weapon`:
                if (weaponHands(this.equips.get($slot `weapon`)) === 1 &&
    ...
    equipFamiliar(familiar) {
        if (!have(familiar))
            return false;
        if (this.familiar && this.familiar !== familiar)
            return false;
    ...
    canEquip(item) {
        if (item === undefined)
            return true;
        if (Array.isArray(item))
            return item.every((val) => this.canEquip(val)); // TODO: smarter
        if (!have(item))
            return false;
        if (item instanceof Item && !canEquip(item))
            return false;
        if (this.avoid && this.avoid.find((i) => i === item) !== undefined)
            return false;
        if (item instanceof Item) {
            ...
            if (slot === $slot `off-hand`) {
                ...
                if (weapon && weaponHands(weapon) === 2)
                    return false;
            }
            if (!this.equips.has(slot)) {
                return true;
            ...
            if (this.familiar && this.familiar !== item)
                return false;
            if (!have(item))
                return false;
            return true;
    ...
    dress() {
        ...
        for (const slot of accessorySlots) {
            const toEquip = accessoryEquips.find((equip) => equippedAmount(equip) < accessoryEquips.filter((accessory) => accessory === equip).length);
            if (!toEquip)
                break;
            const currentEquip = equippedItem(slot);
            //We never want an empty accessory slot
            if (currentEquip === $item `none` ||
        ...
        if (this.modifier) {
            ...
            if (fam_equip !== undefined) {
                ...
                if (index > -1)
                    targetEquipment.splice(index, 1);
            }
